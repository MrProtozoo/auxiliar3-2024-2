...
class UserTest extends FunSuite {
    ...
    override def beforeEach(context: BeforeEach): Unit = {
        ...
        u3 = new User("user123", "123456")
    }

    test("Its obligatory that the user has a username and a password"){
        val u = new User("memeseselmejorramo", "contraseña")
        assertEquals(u.username, "memeseselmejorramo")
    }

    test("Its optional that the user has a name"){
        val u = new User("memeseselmejorramo", "contraseña", "Vicente González")
    ...
    test("Its needed to have a wat to know the number of followers and following"){
        u1.follow(u2)
        u1.follow(u3)
        u3.follow(u2)
        u2.follow(u1)
